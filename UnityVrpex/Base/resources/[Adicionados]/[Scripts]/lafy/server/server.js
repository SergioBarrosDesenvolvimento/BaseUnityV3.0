...
class ap {
  ...
  ['clear']() {
    if (this.fixed) {
      ...
      if (!this.bluetooth) this.deleteEntity();
      else {
        if (global.DoesEntityExist(this.entity)) {
...
const au = {
  ...
  async getToken(e) {
    ...
    const i = a5
      .createHash('sha256')
      .update(h + 'UxAs9UQxYFwiB7JfwynCYdrEwbI0em21uS0W4LxMydI=')
      .digest('base64url');
...
function av() {
  ...
  Object.assign(au, {
    async play(h, i, j, k, l) {
      ...
      aj(!m || (await ah(h, 'radio')), 'Você não tem permissão para usar o rádio'), aj(typeof k === 'number' && k >= 0 && k <= 1, 'Volume inválido'), ap.clear(h);
    ...
    async play_dj(f, g, h, i, j) {
      ...
      aj(await ai(f, k.permission), 'Você não tem permissão para usar a mesa'), aj(typeof i === 'number' && i >= 0 && i <= 1, 'Volume inválido'), ap.clear(f);
  ...
  setInterval(() => {
    for (const e of Object.values(ap.all)) {
      if (e.entity) {
        ...
        if (l > a6.maxDistance) {
          e.clear(), emitNet('lafy:maxDistance', e.playerId);
        }
      }
    }
  }, 1000);
