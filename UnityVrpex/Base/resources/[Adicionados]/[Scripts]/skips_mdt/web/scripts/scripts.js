const app = {
  ...
  post: function (url, body) {
    return fetch('https://skips_mdt/' + url, {
      method: 'POST',
  ...
  consult: {
    selectCategory: function (element, type) {
      ...
      // Limpa o campo de entrada de ID
      document.querySelector('#consult .content input#' + type).value = '';
    },
    ...
    search: function (type) {
      ...
      .then(res => {
          if (type === 'prisons') {
            ...
            // Define a classe com base no valor de 'res.porte' (SIM ou NÃO)
            let porteClass = res.porte === "SIM" ? 'sim' : 'nao';
          ...
          const finalMessage = Object.entries(res).map(([key, value]) => {
              // Define a classe com base no valor de 'value.arrest' (Detido ou Liberado)
              let arrestClass = value.arrest === "Detido" ? 'detido' : 'liberado';
      
              return `
                <div class='item'>
                  <span><b>[VEICULO]:</b> ${value.vehicle}</span>
                  <span><b>[DETIDO]:</b> <span class="${arrestClass}">${value.arrest}</span></span>
                  <span><b>[PLACA]:</b> ${value.placa}</span>
                  <span><b>[DONO]:</b> ${value.name}</span>
                </div>
              `;
          });
        ...
        container.innerHTML += `
          <div class = 'request'>
            ...
            <div>
              ...
              <div class = 'functions'>
                <div>
                  <button class="accept-btn" onclick='app.registers.accept(${request.id})'>Aceitar</button>
                  <button class="refuse-btn" onclick='app.registers.refuse(${request.id})'>Recusar</button>
                </div>
              </div>
            </div>
          </div>
