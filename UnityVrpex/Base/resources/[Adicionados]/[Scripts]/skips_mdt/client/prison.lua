...
function mdt.checkPrisonArea()
    ...
    local _, i = GetGroundZFor_3dCoord(451.4, -1009.79, 18.72)
    local distance = Vdist(x, y, z, 451.4, -1009.79, 18.72, i)
...
function mdt.levarPrisioneiro(prisionTime)
    ...
    if HasModelLoaded(vhash) then
        nveh = CreateVehicle(vhash, 496.27, -1016.59, 28.02, 359.52, true, false)
        ...
        TaskVehicleDriveToCoordLongrange(pveh01, nveh, 1851.04, 2608.18, 45.73, 89.95, 2883621, 1.0)
        ...
        async(function()
            while Transporte do
                ...
                local distancia = GetDistanceBetweenCoords(GetEntityCoords(nveh), 1851.04, 2608.18, 45.73, true)
                ...
                if distancia < 30.0 and math.ceil(vehspeed) <= 2 then
                    if IsPedSittingInAnyVehicle(ped) then
                        ...
                        SetTimeout(
                            3000,
                            function()
                                TaskGoToCoordAnyMeans(ped, 1846.02, 2585.92, 45.68, 0.4, 0, 0, 786603, 0xbf800000)
                ...
                local distanciaPrison = GetDistanceBetweenCoords(GetEntityCoords(ped), 1846.02, 2585.92, 45.68, true)
...
function mdt.prisioneiro(status)
    ...
    if prisioneiro then
        ...
        clearPeds()
        vRP.teleport(1627.15, 2492.52, 45.58)
    end
...
function mdt.setarRoupasPrisional()
    ...
    if GetEntityModel(PlayerPedId()) == GetHashKey('mp_m_freemode_01') then
        ...
        SetPedComponentVariation(PlayerPedId(), 4, 37, 0, 2)
        SetPedComponentVariation(PlayerPedId(), 6, 8, 0, 2)
        SetPedComponentVariation(PlayerPedId(), 7, -1, 0, 2)
        SetPedComponentVariation(PlayerPedId(), 8, 8, 0, 2)
        SetPedComponentVariation(PlayerPedId(), 9, 13, 0, 2)
        SetPedComponentVariation(PlayerPedId(), 11, 154, 0, 2)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(5000)
        if prisioneiro then
            local distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 1627.15, 2492.52, 45.58, true)
            if distance >= 150 then
                SetEntityCoords(PlayerPedId(), 1627.15, 2492.52, 45.58)
                            ...
                            if IsControlJustReleased(1, 51) and segundos <= GetGameTimer() then
                                ...
                                if delayServico[k] == nil or delayServico[k] <= GetGameTimer() then
                                    if vSERVER.checkTempoPrisao() > cfg.geral.tempoMin then
                                        ...
                                        TriggerEvent("Notify","sucesso","Não precisamos mais de seus serviços.", 5)
                                    end
                                else
                                    local delay = delayServico[k]-GetGameTimer()
                                    TriggerEvent("Notify","sucesso","Você já passou por esse local aguarde <b>"..parseInt(delay/1000).." segundo(s)</b>.", 5)
                                end
                            end
                        end
                    end
