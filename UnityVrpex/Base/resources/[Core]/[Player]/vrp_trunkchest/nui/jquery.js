...
const updateChest = () => {
	$.post("http://vrp_trunkchest/requestChest",JSON.stringify({}),(data) => {
		...
		$('#inventory').html(`
			...
			<div class="esquerda">
				${nameList2.map((item) => (`
					<div class="item" style="background-image: url('http://localhost:80/FiveMUnity/cidade/inventory/${item.index}.png');" data-item-key="${item.key}">
			...
			<div class="direita">
				${nameList.map((item) => (`
					<div class="item2" style="background-image: url('http://localhost:80/FiveMUnity/cidade/inventory/${item.index}.png');" data-item-key="${item.key}">
						<div id="peso">${(item.peso*item.amount).toFixed(2)}</div>
						<div id="quantity">${formatarNumero(item.amount)}x</div>
						<div id="itemname">${item.name}</div>
					</div>
